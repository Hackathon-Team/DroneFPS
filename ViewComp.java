...
public class ViewComp extends JComponent { //class declaration for class which controls interactive elements on screen
    ...
    private int[] enemyCounter = new int[3]; //integer array of enemycounter to count down time until respawn
    ...
    public ViewComp() { //constructor
        ...
        for(int x=0; x<3; x++) enemyCounter[x] = 200; //counter is 200 - timer updates every 10, so every 200 updates (2 seconds) monster respawns if it dies
        enemies = new Shootable[3];
    ...
    public void setViewLocation(int c, int y) { //update method called every time timer sends new locations; controlls all movement
        ...
        for(int x=0; x<3; x++) { //for loop to loop through all the enemies
    ...
    public void paintComponent(Graphics g) { //overriding the paintComponent method
        ...
        for(int x=0; x<3; x++) { //for loop to loop through all enemies
